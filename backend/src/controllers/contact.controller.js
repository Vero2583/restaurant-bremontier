...
import { sendContact } from "../config/mailer.js";
...
export const getContact = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Erreur serveur lors de la récupération des contacts" });
  }
};
...
export const createContact = async (req, res) => {
  try {
    ...
    // envoie d'email
    await sendContact(email, nom, email, message);
    ...
    res
      .status(500)
      .json({ message: "Erreur serveur lors de la création des contacts" });
  }
};
